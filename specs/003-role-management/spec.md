...
5. **Given** 系統中存在多個角色，**When** 管理員進入角色管理頁面，**Then** 系統顯示所有角色的列表，包含名稱、描述、建立時間、更新時間等資訊

---

### User Story 2 - 角色權限設定 (Priority: P2)

系統管理員需要為每個角色設定具體的權限，以控制該角色能夠執行的操作範圍。管理員可以在角色詳情頁面勾選或取消勾選各項權限。
...
1. **Given** 管理員進入某個角色的編輯頁面，**When** 勾選或取消勾選權限項目後保存，**Then** 系統更新該角色的權限配置
2. **Given** 某個角色已設定特定權限，**When** 管理員再次進入該角色的編輯頁面，**Then** 系統正確顯示該角色當前已分配的權限（已勾選狀態）
3. **Given** 管理員在角色權限設定頁面，**When** 查看可用權限列表，**Then** 系統以清晰的分類結構展示所有可分配的權限（如按功能模組分組：用戶管理、權限管理、角色管理等）

---

### User Story 3 - 用戶角色分配 (Priority: P2)

系統管理員需要在用戶管理頁面為用戶分配一個或多個角色，使用戶繼承角色的權限。管理員可以在新增或編輯用戶時選擇角色。
...
1. **Given** 管理員在用戶管理頁面新增或編輯用戶，**When** 在角色選擇器中選擇一個或多個角色後保存，**Then** 系統將選定的角色分配給該用戶
2. **Given** 用戶已被分配角色，**When** 管理員在用戶管理頁面查看該用戶資訊，**Then** 系統顯示該用戶當前擁有的所有角色
3. **Given** 管理員編輯某個用戶的角色分配，**When** 移除該用戶的某個角色後保存，**Then** 系統更新用戶的角色配置並移除對應的角色關聯
...
- **角色刪除限制**: 當角色已被分配給用戶時，嘗試刪除該角色應如何處理？系統應阻止刪除並提示「該角色正在使用中，無法刪除」，或者提供選項先解除所有用戶的角色分配後再刪除。
...
- **FR-001**: 系統必須提供角色管理頁面，顯示所有角色的列表，包含角色名稱、角色描述、建立時間、更新時間
...
- **FR-005**: 系統必須在嘗試刪除正在使用的角色時阻止操作並顯示錯誤訊息
...
- **FR-007**: 系統必須提供角色權限設定功能，允許管理員為角色勾選或取消勾選權限項目
- **FR-008**: 系統必須以結構化方式展示可分配的權限列表（例如按功能模組分組：用戶管理、權限管理、角色管理等）
- **FR-009**: 系統必須儲存角色與權限的關聯關係，並在角色編輯頁面正確顯示已分配的權限
- **FR-010**: 系統必須在用戶管理頁面的新增/編輯用戶表單中提供角色選擇器
