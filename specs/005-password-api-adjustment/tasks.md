...
- [X] T029 手動測試管理者重設密碼功能（無需舊密碼）
- [X] T030 手動測試用戶自行修改密碼功能（需提供舊密碼）
- [X] T031 測試密碼驗證規則（長度、複雜度）
- [X] T032 測試舊密碼錯誤處理（僅用戶修改）
- [X] T033 測試版本衝突處理（樂觀鎖）
- [X] T034 測試密碼修改成功後 JWT 失效
- [X] T035 測試非管理者嘗試重設密碼被拒絕（403 Forbidden，驗證後端權限檢查）
- [X] T036 準備 Pull Request 描述與 checklist
