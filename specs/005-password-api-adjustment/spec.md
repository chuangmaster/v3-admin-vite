...
- **FR-011**: 系統必須記錄所有密碼變更操作的審計日誌（包含操作者、時間、目標用戶）**（後端責任）**
- **FR-012**: 管理者重設密碼時必須驗證操作者具有 `account.password.reset` 權限 **（後端責任）**
- **FR-013**: 系統必須在密碼修改成功後更新帳號的 version，使現有的 JWT token 失效

### Assumptions

- 密碼強度規則已在系統中定義（例如：最少 8 個字元，包含大小寫字母和數字）
- 管理者權限驗證機制已存在於系統中（後端會檢查 `account.password.reset` 權限）
- 樂觀鎖機制已在用戶實體中實作
...
- **SC-004**: 100% 的密碼變更操作都被正確記錄在審計日誌中 **（後端驗證項目）**
