...
- **FR-003**: 系統必須強制要求店員上傳身分證明文件（僅接受圖片格式如 JPG、PNG），未上傳則不允許提交
...
- **FR-007**: 系統必須支援線下收購流程：提交表單後產生收購合約與一時貿易申請書，供店員進行網頁簽名，每次簽名結果即時儲存至後台
...
- **FR-012**: 系統必須提供日期區間選擇器供店員選擇寄賣起訖日期，並驗證起始日期不得晚於結束日期
...
- **FR-016**: 系統必須強制要求店員上傳身分證明文件（僅接受圖片格式如 JPG、PNG），未上傳則不允許提交
- **FR-017**: 系統必須提供拍照上傳功能與自動辨識身分證功能（與收購單相同）
- **FR-018**: 系統必須支援線下寄賣流程：提交表單後產生寄賣合約書，供店員進行網頁簽名，簽名結果即時儲存至後台
...
- **FR-032**: 系統必須提供服務單狀態更新功能，狀態選項包含：待處理、已完成、已終止
...
- **FR-036**: 系統必須根據使用者權限控制服務單的建立、查詢、修改、刪除功能
- **FR-037**: 系統必須支援以下權限：`inboundOrder.read`（查詢寄賣單）、`inboundOrder.create`（建立寄賣單）、`inboundOrder.delete`（刪除寄賣單）、`inboundOrder.update`（修改寄賣單）
- **FR-038**: 系統必須支援收購單相關權限（假設與寄賣單權限結構相同但獨立管理）

### Key Entities

- **服務單（Service Order）**: 代表收購單或寄賣單的核心實體，包含服務單類型、服務單來源、客戶資料、商品資訊（品牌、款式、內碼、數量）、金額、建立時間、建立者、修改時間、修改者、狀態等屬性。寄賣單額外包含商品配件、寄賣日期、商品瑕疵處、續約設定等屬性。
