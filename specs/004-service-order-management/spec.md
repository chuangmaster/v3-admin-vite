...
- **FR-005**: 系統必須提供自動辨識身分證功能（&#10022; 星星按鈕），點擊後透過 AI 辨識身分證上的文字，取得姓名與身分證字號
...
- **FR-005-1**: 系統必須在辨識成功後，自動使用身分證字號搜尋既有客戶資料。若找到客戶，自動選擇該客戶並將客戶資訊填入表單；若找不到客戶，將辨識結果（姓名、身分證字號）自動填入新增客戶表單的對應欄位
...
- **服務單（Service Order）**: 代表收購單或寄賣單的核心實體，包含服務單編號（格式：類型前綴 + 日期 + 流水號，如 BS20251214001 或 CS20251214001）、服務單類型、服務單來源、客戶資料、商品項目列表（1-4 件，每件包含品牌、款式、內碼）、總金額、建立時間、建立者、修改時間、修改者、狀態等屬性。寄賣單的每個商品項目額外包含商品配件與商品瑕疵處，整張寄賣單包含寄賣日期與續約設定等屬性。
