...
4. 送出表單以建立收購單。
5. 如果是線下收購，需要把填寫的資料(商品資訊、客戶名稱、收購金額)送至後台系統，後台 API 會提供一份收購合約、一時貿易申請書給使用者進行網頁上的簽名，**兩份文件都需要客戶簽名**，必須分次簽名（先簽收購合約，再簽一時貿易申請書），每次簽名結果傳回後台儲存。
...
- 如果不存在客戶資料，提供新增客戶的選項。
- 表單支援新增多個商品項目，每個服務單最少 1 件，最多 4 件商品。每個商品項目包含品牌名稱、款式、內碼（選填）等欄位。
- 提供新增、編輯、刪除商品項目的功能，並顯示當前商品總件數。
- 表單欄位與寄賣單共用，但不包含商品配件、寄賣日期、商品瑕疵處與續約設定等欄位。
- 上傳文件功能僅能接受圖片格式，如 JPG、PNG 等。
- 提供「拍照上傳」按鈕，方便使用者直接用手機拍攝並上傳身分證明文件。
    - 預覽上傳或是拍照旁邊，有一個按鈕 &#10022; ，點擊後可以自動辨識身分證上的文字（姓名、身分證字號），並自動以身分證字號搜尋既有客戶。
    - 支援辨識台灣身分證（1 英文字母 + 9 數字）與外籍人士居留證。
    - 對於外籍人士若居留證無統一證號或未領有居留證，系統支援手動填寫格式：西元出生年月日（8位數）+ 英文姓名第一個字前兩個字母（大寫），總長度 10 位（例如：19900115JO）。
    - AI 辨識後的自動搜尋流程：
      - 若找到既有客戶：自動選擇該客戶並填入客戶資訊。若辨識的姓名與客戶資料中的姓名不一致，顯示警告訊息但仍可繼續建單。
      - 若找不到既有客戶：將辨識結果（姓名、身分證字號）自動填入新增客戶表單，店員可補充其他資訊後新增客戶。
    - 如果已經手動選擇客戶後才點擊 &#10022; 按鈕，系統會比對辨識的身分證字號與已選擇客戶的身分證字號。若不一致，顯示錯誤訊息並要求重新選擇客戶或重新上傳身分證，不允許繼續建單。
...
| 商品項目   | 商品項目列表（1-4 件）                                                     | 每個服務單可新增 1-4 件商品，每件商品包含以下欄位 |
| - 品牌名稱 | 文字輸入（必填）                                                           | 商品品牌名稱                 |
| - 款式     | 文字輸入（必填）                                                           | 商品款式                     |
| - 內碼     | 文字輸入（選填）                                                           | 內部識別用商品代碼           |
| 總金額     | 數字輸入                                                                   | 收購或寄賣總金額             |
...
- 選擇客戶時，提供搜尋功能以便快速找到既有客戶。
- 如果不存在客戶資料，提供新增客戶的選項。
- 表單支援新增多個商品項目，每個服務單最少 1 件，最多 4 件商品。每個商品項目包含品牌名稱、款式、內碼（選填）、商品配件、商品瑕疵處等欄位。
- 提供新增、編輯、刪除商品項目的功能，並顯示當前商品總件數。
- 寄賣日期欄位需提供日期區間選擇器，預設結束日期為起始日期後 3 個月（90 天）。
...
| - 款式     | 文字輸入（必填）                                                           | 商品款式                     |
| - 內碼     | 文字輸入（選填）                                                           | 內部識別用商品代碼           |
| - 商品配件 | 多選：盒子、防塵袋、購證、提袋、肩帶、羊毛氈、枕頭、保卡、鎖頭/鑰匙、緞帶/花、品牌小卡、保證書、無 | 該商品隨附配件（針對每件商品） |
| - 商品瑕疵處 | 多選：五金生鏽/刮痕/掉、皮質磨損/刮痕/壓痕、內裡髒污、四角磨損            | 該商品瑕疵說明（針對每件商品） |
| 總金額     | 數字輸入                                                                   | 寄賣總金額                   |
| 寄賣日期   | 日期區間                                                                   | 寄賣期間（起訖日期），預設結束日期為起始日期後 3 個月（90 天） |
