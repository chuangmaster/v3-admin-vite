...
- Q: 當管理員刪除用戶時,系統應該如何處理該用戶的資料? → A: 軟刪除(停用)- 保留用戶記錄但標記為「已停用」狀態,用戶無法登入,資料保留用於稽核和歷史記錄
- Q: 匯出報表功能應該生成什麼格式的文件? → A: Excel (.xlsx) - 最常用的企業報表格式,支援格式化、多工作表,易於分享和進一步處理
- Q: 新增用戶時,密碼必須符合什麼安全要求? → A: 中等強度:8字元+混合類型 - 至少8字元,必須包含大小寫字母、數字,符合業界標準
- Q: 用戶列表的分頁功能,每頁應該顯示多少筆資料? → A: 每頁20筆 - 業界標準配置,平衡效能與便利性,適合大多數後台管理系統
- Q: 除了刪除(軟刪除)操作外,管理員是否需要手動啟用/停用用戶帳號的功能? → A: 僅刪除可停用 - 只有刪除操作會標記為停用,無其他狀態切換功能,簡化邏輯
...
2. **Given** 管理員在新增用戶表單，**When** 管理員填寫的資訊不完整或格式錯誤（如郵箱格式錯誤、密碼少於8字元或未包含大小寫字母和數字），**Then** 系統顯示驗證錯誤提示並阻止提交
...
1. **Given** 管理員在用戶管理頁面且擁有刪除用戶按鈕權限，**When** 管理員點擊某用戶的「刪除」按鈕並確認操作，**Then** 系統將該用戶標記為「已停用」狀態並從活躍用戶列表中移除（軟刪除）
...
5. **Given** 用戶被標記為已停用，**When** 該用戶嘗試登入系統，**Then** 系統拒絕登入並顯示帳號已停用訊息

---

### User Story 5 - 匯出用戶報表 (Priority: P3)

管理員需要匯出當前查詢的用戶資料為 Excel 報表文件，以便進行離線分析、存檔或與其他部門分享。
...
1. **Given** 管理員在用戶管理頁面且擁有匯出報表按鈕權限，**When** 管理員執行查詢後點擊「匯出報表」按鈕，**Then** 系統生成並下載包含當前查詢結果的 Excel 文件（.xlsx 格式）
2. **Given** 管理員匯出報表，**When** 報表生成，**Then** Excel 文件包含所有篩選後的用戶資訊和相關欄位，並具備適當的表頭格式和欄位寬度
...
- 當用戶列表資料量極大（如 10,000+ 筆）時，系統如何處理分頁和效能？（每頁顯示20筆，支援快速跳轉到指定頁碼）
...
- 當用戶密碼不符合安全策略時（少於8字元、未包含大小寫字母或數字），系統如何強制執行密碼複雜度要求？
...
- **FR-023**: 被停用的用戶必須無法登入系統，但其資料必須保留用於稽核和歷史記錄
- **FR-024**: 系統必須在用戶列表中明確標示已停用的用戶狀態

### Key Entities

- **用戶 (User)**: 代表系統的使用者，包含用戶名、密碼（加密存儲）、角色、狀態（啟用/已停用）、郵箱、聯絡資訊、創建時間、最後登入時間等屬性。用戶狀態僅透過刪除操作從「啟用」變為「已停用」
...
- **SC-007**: 匯出報表功能可在 5 秒內生成包含 500 筆用戶資料的 Excel 文件
